...
public class JdbcMealRepository implements MealRepository {
    ...
    public Meal get(int id, int userId) {
        List<Meal> list = jdbcTemplate.query(
                ...
                MEAL_BEAN_PROPERTY_ROW_MAPPER, id, userId);
    ...
    public List<Meal> getAll(int userId) {
        return jdbcTemplate.query(
                "SELECT * FROM meals  WHERE user_id=? ORDER BY date_time", MEAL_BEAN_PROPERTY_ROW_MAPPER, userId);
    }

    @Override
    public List<Meal> getBetweenHalfOpen(LocalDateTime startDateTime, LocalDateTime endDateTime, int userId) {
        List<Meal> filteredMealList = jdbcTemplate.query(
                "SELECT * FROM meals WHERE user_id=? AND date_time BETWEEN ? AND ? ORDER BY date_time",
                MEAL_BEAN_PROPERTY_ROW_MAPPER,userId, startDateTime, endDateTime);
